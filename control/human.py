...
def validate_gate(gate: dict[str, Any]) -> None:
    ...
    elif gate["gate_type"] == "GRAPH_APPROVAL":
        if gate["required_state"] != "GRAPH_APPROVED" or gate["subject_type"] != "GRAPH": raise HumanGateError("unsupported graph gate")
        evidence_keys = {"graph_id", "graph_version"}
    ...
    if gate["gate_type"] != "GRAPH_APPROVAL" and not re.fullmatch(r"[0-9a-f]{40,64}", gate["related_candidate"].get("candidate_sha", "")): raise HumanGateError("invalid candidate binding")
...
class HumanGateController:
    ...
    def create_graph_approval(self, *, gate_id: str, project_id: str, intent: dict[str, Any], graph: dict[str, Any], timestamp: str, event_prefix: str) -> dict[str, Any]:
        number = re.sub(r"\D", "", gate_id) or "1"; cap_id = f"CAP-{int(number):03d}"; baseline = "0" * 40
        cap = self.capabilities.issue({"capability_id":cap_id,"schema_version":1,"project_id":project_id,"role":"CONTROLLER","execution_id":self.execution_id,"issued_at":timestamp,"baseline_sha":baseline,"allowed_reads":[".idd"],"allowed_writes":[".idd/human_gates"],"allowed_executes":[],"allowed_operations":["HUMAN_GATE_APPROVAL"],"forbidden_operations":["PROMOTE"],"allowed_request_types":[],"scope_bindings":{"intent_id":intent["intent_id"],"intent_version":intent["version"],"graph_id":graph["graph_id"],"graph_version":graph["version"]},"version":1,"issued_by":"controller"}, event_id=f"{event_prefix}-cap", timestamp=timestamp)
        gate = {"gate_id":gate_id,"schema_version":1,"project_id":project_id,"gate_type":"GRAPH_APPROVAL","subject_type":"GRAPH","subject_id":graph["graph_id"],"required_state":"GRAPH_APPROVED","created_at":timestamp,"status":"WAITING","decision_event_id":None,"related_intent":{"intent_id":intent["intent_id"],"version":intent["version"]},"related_graph":{"graph_id":graph["graph_id"],"version":graph["version"]},"related_eu":{"eu_id":graph["graph_id"]},"related_candidate":{"baseline_sha":baseline},"evidence":{"graph_id":graph["graph_id"],"graph_version":graph["version"]},"approval_capability_id":cap["capability_id"]}
        return self.store.create(gate, event_id=f"{event_prefix}-created", timestamp=timestamp)
    ...
    def decide(self, gate_id: str, *, decision: str, actor_type: str, actor_id: str, binding: dict[str, Any], event_prefix: str, timestamp: str) -> dict[str, Any]:
        ...
        gate = self.store.read(gate_id); current = {"project_root":str(self.project_root),"project_id":gate["project_id"],"execution_id":self.execution_id,"baseline_sha":gate["related_candidate"].get("baseline_sha", "0" * 40),"role":"CONTROLLER"}
        if gate["gate_type"] != "GRAPH_APPROVAL" and binding.get("candidate_sha") != gate["related_candidate"]["candidate_sha"]:
...
class HumanViewGenerator:
    ...
    def generate(self, *, project_id: str) -> dict[str, str]:
        ...
        def gate_line(g: dict[str, Any]) -> str:
            subject = f"candidate `{g['related_candidate']['candidate_sha']}`" if "candidate_sha" in g["related_candidate"] else f"graph `{g['subject_id']} v{g['related_graph']['version']}`"
            evidence = f"Spec `{g['evidence']['spec_result_id']}`, Standards `{g['evidence']['standards_result_id']}`." if g["gate_type"] == "VERIFIED_EU_REVIEW" else (f"Integration `{g['evidence']['integration_result_id']}`." if g["gate_type"] == "INTEGRATION_APPROVAL" else f"Graph `{g['evidence']['graph_id']} v{g['evidence']['graph_version']}`.")
            return f"- **{g['gate_id']}** · `{g['status']}` · subject `{g['subject_id']}` · {subject}\n  - Evidence: {evidence}\n  - Action: approve or reject this exact gate; Markdown is advisory."
        human = page("Current Human Gates", "\n".join(gate_line(g) for g in gates.values()) or "- no gates")
        verification_lines = []
        for g in gates.values():
            if g["gate_type"] == "VERIFIED_EU_REVIEW": verification_lines.append(f"## {g['subject_id']} · `{g['related_candidate']['candidate_sha']}`\n- Spec Verification: PASS (`{g['evidence']['spec_result_id']}`)\n- Standards Review: PASS (`{g['evidence']['standards_result_id']}`)\n- Overall: VERIFIED\n- Human gate: {g['status']}")
            elif g["gate_type"] == "INTEGRATION_APPROVAL": verification_lines.append(f"## {g['subject_id']} · `{g['related_candidate']['candidate_sha']}`\n- Integration Verifier: PASS (`{g['evidence']['integration_result_id']}`)\n- Overall: INTEGRATION VERIFIED\n- Human gate: {g['status']}")
        verification = page("Verification Status", "\n".join(verification_lines) or "- no verified candidate gates")
